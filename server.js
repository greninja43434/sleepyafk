...
function startBot(botConfig) {
  const { id, host, port, username, version, antiAfk, timedMessages, autoRejoin, autoLeave, onJoinCommand, cycleEnabled, cycleLeaveEvery, cycleRejoinAfter } = botConfig;
  ...
  bot.once('spawn', () => {
    ...
    sendOnJoinCommand(id, bot, onJoinCommand);
  ...
  bot.on('kicked', (reason) => {
    ...
    if (autoLeave) { addLog(id,`👢 Kicked (auto-leave): ${msg}`,'warn'); const rt=activeBots.get(id); if(rt){stopIntervals(rt.afkIntervals);stopIntervals(rt.msgIntervals);if(rt.cycleTimeout)clearTimeout(rt.cycleTimeout);} activeBots.delete(id); stopUptimeTicker(id); markStopped(id); io.emit('bot:statusChange',{botId:id,status:'offline'}); return; }
  ...
  bot.on('health', () => { io.emit('bot:stats',{botId:id,health:Math.round(bot.health),food:Math.round(bot.food),ping:bot.player?.ping??0}); });
}
...
app.put('/api/bots/:id', auth, (req,res) => {
  ...
  const {name,host,port,username,version,antiAfk,timedMessages,autoRejoin,autoLeave,onJoinCommand,cycleEnabled,cycleLeaveEvery,cycleRejoinAfter}=req.body;
  bots[idx]={...bots[idx],name,host,port:parseInt(port,10),username,version:version||'',antiAfk,timedMessages,autoRejoin:autoRejoin??false,autoLeave:autoLeave??false,onJoinCommand:onJoinCommand??'',cycleEnabled:cycleEnabled??false,cycleLeaveEvery:cycleLeaveEvery??15,cycleRejoinAfter:cycleRejoinAfter??5};
  writeBots(bots);
